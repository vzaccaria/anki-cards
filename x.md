...
-   For each $g \in G$, one can define an inner automorphism
    $i_g(x) = g x g^{-1}$
...
-   Given a subgroup $(H, \star) \subseteq (G, \star)$, we define the
...
-   Given an action $\curvearrowright: H \times S \rightarrow S$ the
...
-   The set $A(G)=\{ \alpha \}$ is a subgroup of $P(G)$, i.e., the group
...
-   Note for example that $(ab)'=b'a'$ for all elements of a group but
...
An **irreducible** representation $(\rho,V)$ has as subrepresentations
...
-   Given a function $f: G \rightarrow \mathbb{C}$ and a representation
    ...
    Note that since $\rho$ is a matrix of dimension $d \times d$, also
...
-   As it can be equipped with a multiplication (Hamiltonian mult.), it
...
Every polynomial $a(x)$ (with coefficients in $F$) of positive degree
...
-   An extension $F(c_1, c_2)$ can be seen as a vector space of elements
...
One interesting group studied later is the group $G=Aut(L/K)$ where
...
In a finite field of order $q$, the polynomial $X^q - X$ has all $q$
...
An **inner product** space $V$ is endowed with an operation
...
-   Given a co-algebra $C$, $C^*$ is an algebra (over $k^{\star}$). In
...
An Hopf algebra is a bi-algebra with an antipode. An **antipode** $S$ is
...
A poset is a preorder (there can be at most 1 arrow between object) and
...
-   The meet $x_1 \wedge x_2$ exists if it is right adjoint to
    $\Delta: a \mapsto (a,a)$.
...
-   is the unique **coproduct** of two objects in a poset, also called
    **supremum**
-   in boolean algebra, it can be seen as the **or** operation

### Lattice

A lattice is a symmetric monoidal poset where **all objects** have a
**meet** (GLB) and a **join** (LUB). In a lattice, meets and joins:
...
Given a group $G$, we can order subgroups; each pair of them has a
largest common subgroup (intersecting the underlying sets) and a
smallest subgroup containing both (generated by the union of the
underlying sets).

### Symmetric monoidal closed poset and quantales

It is a monoidal poset $$(X, \leq, I, \otimes)$$ with the property that
for any pair of $v,w,a$ elements, there is an element
$(v \Rightarrow w)$ such that:
$$(a\times v)\leq w \textrm{~iff~} a \leq (v \Rightarrow w)$$

For such a poset, $\otimes$ distributes over $\vee$. A poset that has
all joins is called a **quantale**.

### Enriched category

Given :

-   a symmetric monoidal poset $V$ and
-   a set S

A $V$ -category $X$ (or **a category $X$ enriched in \$V\$**) is a
category with:

-   $Ob(X) = S$
-   morphisms $X(x,y) \in V$ (called the hom-objects).

such that $I \leq X(x,x)$ and $X(x,y) \otimes X(y,z) \leq X(x,z)$. A
poset is a Bool category.

### Lawvere\'s Metric space (category)

It is a set $X$ and a function $d: X \times X \rightarrow \mathbb{R}$
such that

-   $d(x,x) = 0$
-   $d(x,y) + d(y,z) \geq d(x,z)$

If Cost is a symmetric monoidal poset $$([0, \infty], \geq, +, 0)$$ we
have that such a metric space is a Cost-category X since

-   $0 \geq d(x,x)$ and
-   $d(x,y) + d(y,z) \geq d(x,z)$.

One can represent this category as a matrix where rows/columns are the
objects and cells are the values of the poset elements.

### Enriched functor

A **V-functor** $F$ from the V-category $X$ to the V-category $Y$ is:

-   a map of objects $F: Ob(X) \rightarrow Ob(Y)$
-   provides for each morphism in $X(x_1,x_2)$ a morphism in
    $Y(F(x_1),F(y_2))$. Since morphisms are elements of the monoidal
    poset $V$, we can see this as a morphism ($\leq$) in the $V$
    monoidal poset, i.e., $X(x_1,x_2) \leq Y(F(x_1),F(x_2))$

### Enriched product category

Given two V-categories $X$ and $Y$, one can form the V-category
$X\times Y$ which is composed of:

-   pairs of objects from $X$ and $Y$ and
-   the hom objects are the objects $X(x_1,x_2) \otimes Y(y_1, y_2)$
    where $\otimes$ is the monoidal product.

### Enriched pro-functor

Given $V$ a commutative quantale, a **V-profunctor** $\Phi$ from the
V-category $X$ to the V-category $Y$ is a **V-functor**:
$$ X^{op} \times Y \rightarrow V$$

For such a functor, one has that:
$$X(x',x) \otimes \Phi(x,y) \otimes Y(y,y') \leq \Phi(x', y')$$

$\Phi$ can be seen as a matrix of elements connecting $X$ and $Y$. If
$X,Y,V$ are Bool, the above builds a new Bool category from two Bool
categories (posets) and it is called a feasibility relation.
...
-   Large category $C$: Either $ob(C)$ or both $ob(C)$ and $hom(C)$ are
    proper classes (i.e., a class that is not a set).

-   Locally small category $C$: $hom(C)$ is a set (**Set** is just an
    example)
...
Given two categories $C$ and $D$, a product category $C \times D$ is
such that
...
### Monoidal and cartesian categories (diagram) {#monoidal-and-cartesian-categories-diagram beamer_opt="fragile"}
...
-   A category $C$ is **monoidal** if:
    ...
    1.  there is a tensor product $\otimes: C \times C \rightarrow C$
        defined for all objects that respects associativity (through
...
-   A category with finite products can always be made into a monoidal
    category by choosing a specific product to be the tensor product

### Operad underlying a symmetric monoidal category [[focus]{.smallcaps}]{.tag tag-name="focus"} {#operad-underlying-a-symmetric-monoidal-category}

Given a symmetric monoidal category $(C,I,\otimes)$, the operad $O_C$ is
defined as the set of:

-   *types* corresponding to the objects of C
-   *operations* of arity $(C_1, \ldots, C_n; D)$ as morphisms
    $C_1 \otimes ... \otimes C_n \rightarrow D$
-   *substitution* on operations
    $(f \circ_i g) = f \otimes (id, id,...,g,...id)$ where
    $g: W \mto C_i$

### Cartesian category

-   It is a monoidal category where the tensor product is a normal
    product defined for all objects and the unit is the terminal object.

-   In a cartesian category, we can \"duplicate and delete information\"
    through the duplication morphism
    $\Delta_X: X \rightarrow X \times X$ and the map into the terminal
    object $!_X: X \mto 1$

### Closed category

-   In a closed category there is also an object representing **all**
    the morphisms from $X$ to $Y$ denoted $Y^X$. In a closed category,
    we have: $$hom(X \otimes
     Y, Z) \cong hom(Y, Z^X)$$ The natural isomorphism is called
    **currying**.
...
-   Visually, there is a one to one correspondence
    ...
    ::: CENTER
    ```{=latex}
    \includegraphics[width=4cm]{./key/page-2-crop.pdf}
    ```
    :::

### Closed category - name of a morphism

-   The **name** of a morphism $f: X \mto Y$ is a morphism
    $[f]: I \mto Y^X$, i.e., an element of $Y^X$

-   One can recover the original $f$ with the **evaluation** morphism
    $ev$: $$ f = ev \circ (Id \otimes [f]) \circ r^{-1}$$ where
    $r^{-1}: X \mto X \otimes I$.

-   The evaluation map is the uncurrying of the identity on $Y^X$

    ::: CENTER
    ```{=latex}
    \includegraphics[width=3cm]{./key/page-1-crop.pdf}
    ```
    :::

### Compact closed category - name of a morphism

-   The **name** of a morphism $f: X \mto Y$ is a morphism
    $[f]: I \mto X' \otimes Y$ (the latter in a CCC is the exponential
    object)
...
::: CENTER
```{=latex}
\includegraphics[width=3cm]{./key/page-3-crop.pdf}
```
:::
...
A compact (or *autonomous*) category is such that every object $X$ has a
dual $X'$:

::: CENTER
```{=latex}
\includegraphics[]{./images/compactdual.pdf}
```
:::

and there are unit $i_X: I \cong X \otimes X'$ and co-unit
$\epsilon_X: X
\otimes X' \cong I$ natural transformations that satisfy the zigzag
equations:
...
-   A compact category is automatically closed, i.e., the exponential
    object be defined as $$Y^X = X' \otimes Y$$ and it can be shown that
    it satisfies currying. The currying isomorphism assumes a particular
    form as the following two diagrams are equivalent:

    ::: CENTER
    ```{=latex}
    \begin{figure}
    \subfigure{\includegraphics{./images/cchom1.pdf}}
    \subfigure{\includegraphics{./images/cchom2.pdf}}
    \end{figure}
    ```
    :::
...
A dagger category is a category $C$ which has a contravariant functor
$\dag$ that is identity on objects and is such that
$$\vdag{(\vdag{f})} = f$$ for all morphisms. Any morphism $f$ in the
category Hilb has a $\vdag{f}$ defined as: $$\ang{\vdag{f}w,v} = 
\ang{w,fv}$$. It is called the **Hilbert space adjoint**.

### Cartesian closed category

It is a cartesian category that has:

-   the terminal object (the unit of the product)
-   a product of any pair of object
-   exponentials for any pair of objects

### Powers in a bicartesian closed category (Set) [[focus]{.smallcaps}]{.tag tag-name="focus"} {#powers-in-a-bicartesian-closed-category-set}

-   **zeroth power**: the set of morphisms that go from the initial
    object to any object and get the terminal object $a^0 = 1$

-   **powers of one**: there is only one morphism that goes from any
    object to the final one: $1^a = 1$

-   We also have that: $(a^b)^c = a ^{b\times c}$ and
    $(a\times b)^c=a^c\times b^c$

### Bicartesian closed category

It is a category which is cartesian in both (product, terminal) and
(coproduct, initial) settings and in which the product can be
distributed over the co-product:
$$a \times (b+c) = a\times b + a \times c$$

### Monoidal category of endofunctors

Endofunctors and natural transformations form a category. If there are
additional natural transformations of the type $T\times T \rightarrow T$
then this category is a monoidal category. These monoids are called
monad.

### Category of objects under object $b$ (coslice category) {#category-of-objects-under-object-b-coslice-category beamer_opt="fragile"}

Assume $b$ is an object of category $C$ and construct the category of
objects under $b$ called $(b \downarrow C)$ with objects all pairs
$\np{f}{c}$ where $f$ is an arrow $f: b \rightarrow c$. Morphisms
between these objects are those that make the following diagram commute:
...
One might define also the category of *objects over \$a\$* called
$(C \downarrow a)$ by taking the opposite arrows.

### Category of pointed sets $Set_{\star}$ (example of co-slice category)

In this category,

-   An object is a pointed set $(A \in Set,\nu_a)$ with a distinguished
    point $\nu_a \in A$.

-   A morphism $f: A \rightarrow B$ is a morphism that is *point
    preserving* if $f(\nu_a) = \nu_b$

-   This category corresponds to $(\star \downarrow Set)$ where $\star$
    is the singleton set, as arrows between $\star$ and any set
    corresponds to picking element $\nu_A$ in it.

### Category of objects (functor)-under object $b$ {#category-of-objects-functor-under-object-b beamer_opt="fragile"}

Given the functor $S: D \rightarrow C$, the category of objects
*S-under* $b$ ($b
\downarrow S$) is the set of all pairs $\np{f}{d}$ and morphisms $h$
...
### Category of objects (functor)-under object $X$, example $Grp \rightarrow Set$ {#category-of-objects-functor-under-object-x-example-grp-rightarrow-set beamer_opt="fragile"}
...
-   The category ($X \downarrow S$) has objects $\np{G}{i_{GX}}$ where
    $i_{GX}$ maps $X$ into the underlying set of $G$ while morphisms $h$
    are those that make the above diagram commute.

-   The *free group* on X ($F(X)$) is the initial object in this
    category

### Product and permutation category (PROP)

-   A PROP is a symmetric monoidal category with objects the natural
    numbers, the monoidal operator $\oplus$ is addition.

-   They are used to describe a given sort of algebraic structure. Every
    object is of the form
    $$x^{\otimes n} = x \otimes x \otimes \cdots \otimes x$$

### Product and permutation category **FinSet**

-   **FinSet** is a PROP where morphisms $f: m \mto n$ are functions
    from the set $\ubr{m} = \{1 \ldots m\}$ to the set
    $\ubr{n}=\{1 \ldots n\}$.

-   The monoidal product of two morphism is obtained by pasting one
    above the other the two original morphism and the inputs and output
    set.

### Product and permutation Bij

A PROP **Bij** has morphisms $f: m \mto n$ bijections
$\ubr{m} \mto \ubr{n}$. Note that for $m \neq n$ the homset is empty.
The morphisms are just permutations.

### Product and permutation $Rel_{Fin}$

Morphisms $m \mto n$ are relations $R \subseteq \ubr{m} \times \ubr{n}$.
The composition $R.S$ ($S \subseteq \ubr{n} \times \ubr{p}$) is the set
of pairs $(i,k) \in \ubr{m} \times \ubr{p}$ for which
$$\exists j. (i,j) \in R \wedge (j,k) 
\in S$$

### Symmetric monoidal theory (presentation for a PROP)

-   A **symmetric monoidal theory** (or *presentation*) for any PROP $G$
    is a pair $(\Sigma, E)$ where $\Sigma$ is the set of morphisms
    (generators) of type $n
     \rightarrow m$ other than $id$ and $\sigma$, while $E$ is a set of
    equations over $\Sigma$.
-   A $\Sigma$ term is any morphism that can be built by composing the
    generators.

### Freely generated PROP

Given a symmetric monoidal theory $(\Sigma, E)$, a **freely generated
PROP** $G$ is the set of expressions that one can construct with
elements in $\Sigma$ quotiented over $E$ (and the axioms of PROP)

### SMT for a monoid {#smt-for-a-monoid beamer_opt="fragile"}

-   For a monoid $(M, m, 1)$, an SMT $(\Sigma, E)$ is composed of the
    following equations (white dot is multiplication).

    ::: CENTER
    ```{=latex}
    \includegraphics[width=10cm]{./images/monoid.png}
    ```
    :::

-   The freely generated PROP $\MM$ is isomorphic with FinSet, so we say
    that the theory of commutative monoids **presents** FinSet.
...
::: CENTER
```{=latex}
\includegraphics[width=10cm]{./images/comonoid.png}
```
:::

### SMT for a bi-algebra

The operations of a co-monoid are monoid morphisms:

::: CENTER
```{=latex}
\includegraphics[width=10cm]{./images/bialgebras.png}
```
:::

-   (A7) is $\epsilon(m(a \otimes b)) = \epsilon a \otimes \epsilon b$
-   (A8) is
    $\Delta \circ m = (m \otimes m) \circ (id \otimes \tau \otimes id)
     \circ (\Delta \otimes \Delta )$

### SMT for a special frobenius algebra

A Frobenius algebra is a second way (other than bi-algebra) to combine a
monoid and a co-monoid:

::: CENTER
```{=latex}
\includegraphics[width=10cm]{./images/frobenius.png}
```
:::

-   (F1 - pseudo-commutativity)
    $\Delta \circ m = (m \otimes id) \circ (id \otimes \Delta)$
-   (F2 - factorization) says that co-multiplication factorises the
    original element
...
-   For example one can create a graph PROP G = SB + N, where SB
    (*special bialgebra*) gives the branching structure while $N$ is a
    *node* PROP whose signature is a single $1 \mto 1$ morphism (node)
    and no equations.: So an example of morphism in $G = SB+N$ would be:

    ::: CENTER
    ```{=latex}
    \includegraphics[width=4cm]{./images/prop_sum_graph_prop.pdf}
    ```
    :::

## Set category [[focus]{.smallcaps}]{.tag tag-name="focus"} {#set-category}

### Monomorphisms (Sets) {#monomorphisms-sets beamer_opt="fragile"}
...
### Epimorphisms (Sets) {#epimorphisms-sets beamer_opt="fragile"}
...
-   If $f$ is not surjective, there are elements in $C$ which will not
    participate to $g_{*} \circ f$ (*terra incognita*). There will be
    thus $g_1$ and $g_2$ that differ only in terms of those excluded
    terms while their composition is the same.

### Terminal object (Sets)

There is a set 1 for which, for any set $X$, there is a unique function
$X \rightarrow 1$. This is called the **terminal object**.

### Unit of categorical product (Sets)

The unit of a categorical product is the terminal object,
$X \times 1 \simeq X$
...
-   and there is $k: 1 \rightarrow B$ such that $x: 1 \rightarrow X$
    factors through $m$, i.e., $x = m \circ k$

then $x \in B$

### Subobject (Sets) {#subobject-sets beamer_opt="fragile"}

A subobject of $c$ is an equivalence class $S=\{v, w \ldots \}$ where
$v,w \ldots$ are monos from objects connected by isomorphisms.

::: CENTER
```{=latex}
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRACMQBfU9TXfIRQBGclVqMWbOt14gM2PASKjh4+s1aIQAY27iYUAObwioAGYAnCAFskZEDghIATNRx0sDNgAsIEAGsQag0pbRpZC2s7RFFHZ0Q3R09vbT9A4IlNNgB3SJArW3t3BLifGDooNkgwVhDJLRAAHSbsOy4KLiA
\begin{tikzcd}
b \arrow[rd, "v", hook] \arrow[r, "\sim", no head] & a \arrow[d, "w", hook] \\
                                                   & c                     
\end{tikzcd}
```
:::
...
Given two functions ($g_1, g_2: X \rightarrow Y$), **their equaliser**
is an **object** and **monomorphism** **pair** $(E,m: E \rightarrow X)$
for which the following properties hold:

1.  **Equivalence**: $g_1 \circ m = g_2 \circ m$
2.  **Limit**: for any other object pair $(O,m_o: O \rightarrow X)$
    where $g_1 \circ
      m_o = g_2 \circ m_o$, there exists a unique morphism
    $f: O \rightarrow E$ such that $m_o = m \circ f$

$E$ should be understood as the subset of elements of $X$ for which
$g_1(x) =
g_2(x)$, i.e., the solutions of the equation.

### Function objects (Sets) {#function-objects-sets beamer_opt="fragile"}

-   The object-morphism pair
    $(b^{a}, eval: b^{a} \times a \rightarrow b)$ makes an **exponential
    object** if for all other object-morphism pair $(z \times a, e)$
    there exists a mapping $\lambda(e)$ such that the following
    commutes:
...
-   the mapping $\lambda_e: z \rightarrow b^{a}$ is called the called
    **currying** of $e: z \times a\rightarrow b$.

-   in fancy words, the exponential object is a universal arrow to $b$
    of a functor $H[-] = - \times a$

### Zero morphism {#zero-morphism beamer_opt="fragile"}

-   A left-zero morphism $0_{XY}$ is such that the following commutes
    for any $W, g, h$
...
-   A right-zero morphism is such that the following commutes for any
    $Z,g,h$
...
### Kernel object {#kernel-object beamer_opt="fragile"}

-   A kernel object of a morphism $f: X \rightarrow Y$ is the equalizer
    of $f$ and a right-zero-morphism $0_{XY}: X \mto Y$
...
-   In the category of groups, it is exactly the kernel of a morphism of
    groups

## Kleisly category and monads [[focus]{.smallcaps}]{.tag tag-name="focus"} {#kleisly-category-and-monads}

### Kleisly category ($C_T$) definition

-   Assume $C$ is a category with an endofunctor $T$ and a morphism
    $\mu: T^2 C \rightarrow C$
-   $C_T$ has the same object as $C$ but any morphism
    $A \rightarrow_T B$ is built by picking a morphism
    $A \rightarrow T B$ in the following way:
    -   The **identity** for any $A$ is constructed by picking a
        morphism $\eta_A: A
         \rightarrow T A$
    -   The composed arrow $h_T = f_T \circ g_T : A \rightarrow_T C$ is
        the the one built as $h: A \rightarrow T C$ such that $$ h = \mu
         \circ T f \circ g$$
...
-   a unit natural transformation $\eta: 1_C \Rightarrow T$ (where $1_C$
    is the identity endofunctor)
-   a multiplication natural transformation $\mu: T^2 \Rightarrow T$
    arising from functor composition so that, seeing the endofunctors as
    objects in the monoidal category of endofunctors $C^C$, the unit and
    the multiplication can be used to make $T$ a monoid. Recall that,
    multiplication is functor composition.

### Monads from adjoint functors {#monads-from-adjoint-functors beamer_opt="fragile"}
...
\end{tikzcd}
```
-   A monad is the shadow cast by an adjunction into the domain of the
    left-adjoint $C$.
-   It raises from natural transformations that one can build on the
    base functor $T=RL$.
-   The adjunction ensures that $\mu: RLRL \rightarrow RL$ is natural.

### Monads from adjoint functors (Maybe monad) {#monads-from-adjoint-functors-maybe-monad beamer_opt="fragile"}
...
-   The left-adjoint is **free** and takes
    -   each set $S$ to $(S \cup \{\nu_S\}, \nu_S)$.
    -   each $f: S_1 \rightarrow S_2$ to a function $f^*$ such that
        $f^*(s)=f(s), f^*(\nu_A) = \nu_B$
-   The right-adjoint maps to $S \cup \{\nu_S\}$ and $f^*$.
-   $\eta = LR$ adds a new basepoint to the set and the natural
    transformation is given by natural inclusion.
-   $\mu = LRLR$ map the two new points (because we have to LR in
    series) to a single given by $LR$.

### Monad laws (left identity - bind) {#monad-laws-left-identity---bind beamer_opt="fragile"}
...
\end{tikzcd}
```
$f^o$ is the bound function. It should commute with the return of the
monad.

### Monad laws (right identity - return) {#monad-laws-right-identity---return beamer_opt="fragile"}
...
### Monad laws (bind associativity) {#monad-laws-bind-associativity beamer_opt="fragile"}
...
-   Given categories C and D, a *D-indexed monad $T$ over C* is a
    C-endofunctor $D^D
...
-   For example, in Haskell, $D=\{lock, unlock\}$ and it becomes easy to
    see see if the computation acquires the lock, or does not touch the
    lock.
...
A functor $$ F : \mathcal{C} \rightarrow \mathcal{C'} $$ is a pair
$(F_o, F_m)$ where

-   $F_o$ maps **each object** in $\mathcal{C}$ into an object in
    $\mathcal{C'}$, while
...
A **full functor** $T: C \rightarrow D$ is an **epimorphism** between
morphisms in $C$ and $D$.

### Faithful functor

A faithful functor $T: C \rightarrow D$ is a **monomorphism** between
morphisms in $C$ and $D$.
...
A constant functor $\Delta_c: B \rightarrow C$ is a functor that maps
every object in $B$ into a single object $c \in C$ and every morphism
into the identity map on $c$.

### Bifunctors

-   A bifunctor over $C$ and $D$ is a functor over pairs of objects and
    morphisms, i.e., $C \times D \rightarrow E$

-   Products and co-products are special bifunctors
    $C \times C \rightarrow C$

### Contravariant functors {#contravariant-functors beamer_opt="fragile"}

-   A contravariant functor is a functor $C^{op} \rightarrow D$. It
    basically maps inverse arrows in $C$ to $D$.
...
-   An example is the functor $F[-] = Q^{(-)}$ which is indexed by an
    object $Q$ and maps each object $c$ to the set of morphisms from $c$
    into $Q$.
...
-   If $F_1(X)$ and $F_2(X)$ are functors then $T(X) = F_1(X) + F_2(X)$
    is a functor
...
Thus any polynomial expression in an object $X$ can be made into a
functor.
...
-   Proving a logic predicate means constructing an element of a
    specific type
...
It is a way of comparing functors; given two functors
$F,G: C \rightarrow D$, I can create compare them by :

-   picking to objects object $c_1,c_2$ and a morphism
    $f: c_1 \rightarrow c_2$
-   picking in $D$ a morphism $\alpha_{c_1}$ in $D$ that maps $Fc_1$ to
    $Gc_1$ (the family of $\alpha_c$ is called **components of the
    NT**).
-   picking in $D$ a morphism $\alpha_{c_2}$ in $D$ that maps $Fc_2$ to
    $Gc_2$

How can I detect any relation between $F,G$ meaning they are similar?
This naturality condition is
$$\alpha_{c_2} \circ F f = G f \circ \alpha_{c_1},
    \forall c_1,c_2$$

### Natural transformation maps morphisms to ...

A natural transformation maps a morphism to a commuting diagram
(**naturality square**).
...
-   If I compose the components of two natural transformations
    $\alpha: F a
     \rightarrow G a$ and $\beta: G a \rightarrow H a$, do I have
    another natural transformation? yes
-   Is there an identity NT? yes, it is the family of identity morphisms
    $F a \rightarrow F a$
-   The category of functors from $C$ into $D$ is called $D^C$
-   In \'Cat\', the morphisms between categories (functors) are not only
    a set but a category

### Natural transformation vertical composition {#natural-transformation-vertical-composition beamer_opt="fragile"}
...
### Natural transformation horizontal composition [[focus]{.smallcaps}]{.tag tag-name="focus"} {#natural-transformation-horizontal-composition beamer_opt="fragile"}
...
$$(\gamma * \alpha)_c = \gamma_{Gc} \circ H\alpha_c = K\alpha_c \circ \gamma_{Fc}$$

When some ntt. are identities, these are called whiskerings, e.g.,
$H\alpha:
...
It can be shown that $\gamma * \alpha$ can be decomposed into
whiskerings of either $\alpha$ or $\gamma$.
...
-   A lot of parametric polymorphic functions `F a -> G a` are natural
    transformations by default (theorems for free) and can be used to
    optimize code, e.g.:
    $$safeTail \circ fmap~f = fmap~f \circ safeTail$$
...
-   For all 0-cells $A$ and $B$, there is a category of morphisms
    $C(A,B)$ where:

    -   objects are the morphisms between $A$ and $B$ (called 1-cell)
    -   morphisms are morphisms between morprhisms and are called
        2-cells.

-   Example: CAT, where 1-cells are functors and 2-cells are natural
    transformations

### Bi-category example **Cat**

-   0-cells are categories
-   1-cells are functors and they must compose (symbol used for
    composition is \";\")
...
-   1-cell $x \mto y$ is a span $(z, f: z \mto x, g: z \mto y)$;
    composition is given by pullback

::: CENTER
```{=latex}
\includegraphics{./images/spanc_2cellcomp.pdf}
```
:::
...
-   Horizontal composition of two 2-cell $h, h'$ is such that the
    diagram on the right commutes

    ::: CENTER
    ```{=latex}
    \begin{figure}
    \subfigure{\includegraphics{./images/spanc_2cell.pdf}}
    \subfigure{\includegraphics{./images/spanc_2cellhcomp.pdf}}
    \end{figure} 
    ```
    :::
...
-   Vertical composition (2-cells along one cells) is composition in $C$
    ...
    ::: CENTER
    ```{=latex}
    \includegraphics{./images/spanc_2cellvcomp.pdf}
    ```
    :::

### Monad in a bicategory {#monad-in-a-bicategory beamer_opt="fragile"}

**A monad on an object \$x\$** of the bicategory $B$ is a 1-cell
$F: x \mto x$ with 2-cells $\eta$ and $\mu$ such that the following
commute:

::: CENTER
```{=latex}
\includegraphics[width=10cm]{./images/bicategory_monad.pdf}
```
:::

If $B=Cat$, $x$ is a category and the above corresponds to the usual
notion of monad as an endofunctor. A monad morphism $\theta$ is a 2-cell
making the following commute:

::: CENTER
```{=latex}
\includegraphics[width=8cm]{./images/bicategory_monad_morphism.pdf}
```
:::

### Monad in a bicategory: example of **Span(Set)**

-   In $Span(Set)$, a monad $F$ on an object $Ob$ is a 1-cell (morphism)
    $Ob \lto
     Ar \mto Ob$; $\mu$ associates to each $f, g \in Ar$ their
    composition $f; g
     \in Ar$ while $\eta$ associates to each $x \in Ob$ an arrow
    $a \in ar$

    ::: CENTER
    ```{=latex}
    \begin{figure}
    \subfigure{\includegraphics[width=5cm]{./images/spanset_mu.pdf}}
    \subfigure{\includegraphics[width=2cm]{./images/spanset_eta.pdf}}
    \end{figure} 
    ```
    :::

-   Given a monad $F$ over two sets $Ob$ and $Ar$, these can be made
    into a small category $C_F(Ob,Ar)$.

### Monad in a bicategory: example of **Span(Mon)**

-   In $Span(Mon)$, where $Mon$ is the category of monoids and monoid
    homomorphisms, a monad over a monoid $m$ can be used to define a
    small strict monoidal category with objects the elements of $m$.

### Monad in a bicategory: monad composition in **Span(Set)**

-   Given to monads $F$ and $G$ over a single set $Ob$ and a 2-cell
    $\lambda: F;G
     \mto G;F$) respecting some additional rules and called
    *distributive law*

-   Then one can create a monad $G;F$ over $Ob$

## Representability and universal properties

### Representable functors {#representable-functors beamer_opt="fragile"}
...
\end{tikzcd}
```
-   Given an object $d \in C$, we have a functor
    $d^{(-)}: C^{op} \rightarrow Set$

-   Given a generic contravariant functor $F: C^{op} \rightarrow Set$,
    we say that it is **representable** by $d$ if it is isomorphic to
    $d^{(-)}$.
...
-   Works if you think about (`Rep f)` as the index into the container
    (`f x)`. For example streams are representable while lists are not.
    `tabulate` creates the container, `index` extracts a value from a
    value of `Rep f`.

-   Another example is the matrix functor that is representable by a
    pair of integers. A morphism from `(int,int) -> a` is one on one
    with a matrix `Matrix a`.
...
If there exists an object $X$ for which $(-)^X$ is isomorphic to the
functor

$$\textrm{sing} : (-) \mapsto \{ \cdot \}$$

that maps objects to the singleton set, then we say that $X$ is
**initial.** We also say that sing is representable by $X$.
...
This means that, the set of morphisms into $X$ from any other object is
singleton.

### Universal property encoded by $Id_{Set}: Set \rightarrow Set$ {#universal-property-encoded-by-id_set-set-rightarrow-set beamer_opt="fragile"}

Take $Id_{Set}$ as the **identity functor** in the category $Set$ then,
...
The **singleton set** is the only value for (?) that represents
$Id_{Set}$ because only for that object there is a natural
transformation that makes the right square commute.

### Universal property encoded by the forgetful functor $U: Grp \rightarrow Set$ {#universal-property-encoded-by-the-forgetful-functor-u-grp-rightarrow-set beamer_opt="fragile"}
...
\end{tikzcd}
```
The only value for $?$ is the group $Z$. i.e., the **free group on a
single generator**. So $Z$ represents $U$. In fact, the set $G^Z$ is the
set of homomorphisms from $Z$ to $G$ whose size depends on which of the
elements of $G$ we map the generator.

### Universal arrow from object {#universal-arrow-from-object beamer_opt="fragile"}

-   A **universal arrow from object** $c \in C$ to $S$ allows to factor
    all morphisms $f'$ through morphisms in the codomain of incoming
    functor $S: D \rightarrow C$.

-   It is an object/morphism pair $(r, u)$ such that the following
    diagrams commute.
...
### Universal arrow from object (coslice category view definition) {#universal-arrow-from-object-coslice-category-view-definition beamer_opt="fragile"}

Given the functor $S: D \rightarrow C$, we can define a universal arrow
from object $c$ to $S$ as the initial object $\np{r}{u}$ in the coslice
category $(c
\downarrow S)$
...
### Universal arrow from object and representation {#universal-arrow-from-object-and-representation beamer_opt="fragile"}

$(r, u)$ is universal for $(S,c)$ means that $(S-)^c$ **is represented**
by $r$.
...
### Universal arrow from object (free vector spaces) [[focus]{.smallcaps}]{.tag tag-name="focus"} {#universal-arrow-from-object-free-vector-spaces beamer_opt="fragile"}

-   $U\KK[X]$ is the set of vectors in $\KK[X] = span(X)$.
-   $j_X$ injects the basis into the whole set $U\KK[X]$. It is
    universal.
-   Any (linear) $f: \KK[X] \rightarrow W$ can be defined by choosing
    $g$ (**extending it**) $$Uf \circ j_X = g$$
...
$j_X$ could be thought of as the cases where the input parameter of
$U_x$ are just those in $X$. If $f$ is linear, it suffices to specify
its value only for those cases.

### Universal arrow from object (free group) [[focus]{.smallcaps}]{.tag tag-name="focus"} {#universal-arrow-from-object-free-group beamer_opt="fragile"}

-   A **free group** $F_X$ on the set $X$ is a universal arrow from $X$
    to the group forgetful functor. It is a pair $(F_X, u)$ such that
    for any function $v: X \mto |G|$, there is a unique group
    homomorphism $f$ for which the diagram below commutes.
...
### Universal arrow $S_G \mto S_{G/N}$ to forgetful $Grp \mto Set$ resp. kernels {#universal-arrow-s_g-mto-s_gn-to-forgetful-grp-mto-set-resp.-kernels beamer_opt="fragile"}

$(\pi_{G,N}, G/N)$ is the universal arrow to $H: Grp \rightarrow Set$
(maps respecting the kernel). Any mapping $x_k: S_G \rightarrow S_X$ can
be fixed with $f_k$. Or, fixing $x_k$ gives a group hom $f_k$.
...
### Universal arrow $S \mto US^*$ to forgetful $Set_* \rightarrow Set$ {#universal-arrow-s-mto-us-to-forgetful-set_-rightarrow-set beamer_opt="fragile"}
...
\end{tikzcd}
```
### Universal element {#universal-element beamer_opt="fragile"}
...
Assume a functor $H: \mathbb{U} \rightarrow \mathbb{SET}$. $(U, u)$ is a
universal element of $H$ if, for every element $x_k$ in $HX$ there
exists a unique $f_k$ such that the above diagram commutes. Practically,
it enables a unique correspondence between elements of a set and
morphisms.

### Universal element (Sets and equivalence relations) {#universal-element-sets-and-equivalence-relations beamer_opt="fragile"}

-   Given an equivalence relation $E$ over $f$, the familiar mapping
    $p: S
     \rightarrow S/E$ is a *universal element* of the functor $H_{/E}$
    which maps to morphisms of maps preserving the equivalence relation.

-   $p$ is universal, because for any element $x_k$ there exists a
    unique $f_k$ such that this diagram commutes:
...
### Universal element (Groups and quotients) {#universal-element-groups-and-quotients beamer_opt="fragile"}

Given a functor $H=(-)^G_N : Grp \rightarrow Set$ giving the sets of
homomorphisms from $G$ for which $fN=1$ and $p$ as the mapping into
cosets $G \rightarrow G/N$ then $(G/N,p)$ are a universal element of
$H$.
...
### Universal arrow vs universal element {#universal-arrow-vs-universal-element beamer_opt="fragile"}
...
Considering the category of pointed sets and a generic functor, a
universal element $(U,u)$ is exactly a universal arrow from $*$ to $H$.

Seeing this as a coslice category $(1 \downarrow Set)$, every morphism
$1 \rightarrow HX$ is in direct correspondence with $f_x$ if it makes
the coslice category diagram commute.

### Universal arrow to object {#universal-arrow-to-object beamer_opt="fragile"}

-   A **universal arrow to object** $c \in C$ from $S$ is an
    object/morphism pair $(r,
     v)$ such that, in the following diagram, for any $f$ there is an
    $f'$ such that it commutes
...
### Universal arrow to object, product example [[focus]{.smallcaps}]{.tag tag-name="focus"} {#universal-arrow-to-object-product-example beamer_opt="fragile"}
...
Projection functions $\ang{p, q}$ from the product $a \times b$ (and the
product itself) are the universal arrow from the functor
$\Delta(c) = \ang{c,c}$ to the object $\ang{a,b}$.

### Presheaf

A presheaf on a small category $C$ is a functor $$C^{op} \mto Set$$. The
category of presheaves on $C$ is denoted as $[C^{op}, Set]$ and has
functors as objects and natural transformations as morphisms.

## Monad algebras

### Drawbacks of F-algebras

-   Given an algebra $(X, \theta: T(X) \rightarrow X)$, with
    $T(X) = 1 + X^2$ we know that $\theta$ has the same signature of a
    monoid but it should not abide by its laws. Not every such algebra
    is a monoid.

-   Monad algebras can help enforce additional structure on functor
    algebras

### Monad algebras {#monad-algebras-1 beamer_opt="fragile"}

Given a monad $(T,\eta,\mu)$, a **monad algebra** is an algebra of the
underlying functor $(X,\theta)$ where, additionally, the following
diagrams commute:
...
-   The **Eilenberg-Moore** category is built with object $(X,\theta)$
    that respect the above laws (morphisms are F-algebra morphisms).

-   Practically, a monad algebra enforce monoidal laws on $\theta$.

## Adjunctions

### Adjoint functors {#adjoint-functors beamer_opt="fragile"}
...
\end{tikzcd}
```
-   Two categories $C$ and $D$ are equivalent if I can build two
    functors, one going left $L: D \leftarrow C$ and
    $R: D \rightarrow C$ if their composition is naturally isomorphic,
    through $\eta$ and $\epsilon$, to the identity functor.
-   We say that $L$ is *left adjoint* to R

### Adjoint functors and homsets

-   It is another way of stating that two functors $L: D \leftarrow C$
    and $R:
     D \rightarrow C$ are adjoint by comparing homsets
    $$D(L c, d) \cong C(c, R
     d)$$ for all $c,d$.
-   If this happens, then $$C(c,RLc)\cong D(Lc, Lc)$$ but $C(c, RLc)$ is
    our family of mappings/natural transformation $\eta$, if it exists.
    And it does, because $D(Lc,Lc)$ is not empty.

### Exponentials - currying [[focus]{.smallcaps}]{.tag tag-name="focus"} {#exponentials---currying beamer_opt="fragile"}

An exponential object $b^a$ is the universal arrow from the
$(- \times a)$ functor to $b$ and is a bijection of homsets
$e \leftrightarrow \lambda_e$.
...
This can also be seen as two adjoint endofunctors in $C$; the left
adjoint is $-
\times a$ while the right adjoint is $-^a$. In particular, $eval$ is the
co-unit of the adjunction (i.e., a morphism: $RL \rightarrow I$)

### List monad from adjoint functors {#list-monad-from-adjoint-functors beamer_opt="fragile"}

-   $L$ maps a set $X = \{ x_i \}$ into the free monoid of words built
    by mapping each element into a letter $\ang{x_i}$.

-   $U$ is the forgetful functor that produces a set of words
    $\{ \ang{a},
...
-   The adjunction $L \ladjof U$ produces the list monad $T=UL$ over set
    where:

    -   $\eta_X$ maps each $x \in X$ to $\ang{x}$ in $ULX$ (the set
        produced by the forgetful functor).
    -   $\mu_X$ flattens the list, e.g.,
        $\mu_X(\ang{a\ang{ab}}) = \ang{aab}$

### Pre - Yoneda Lemma - G-Equivariant maps {#pre---yoneda-lemma---g-equivariant-maps beamer_opt="fragile"}

Given two actions of a group $G$ (interpreted as functors) over two sets
$X$ and $Y$, a natural transformation among them is called a **$G$
equivariant map** $\alpha$
...
### Pre - Yoneda Lemma - Representability of functor from $BG$ to $G$ {#pre---yoneda-lemma---representability-of-functor-from-bg-to-g beamer_opt="fragile"}

$\vert G \vert$ can be seen as an object in Set over which $BG$ acts
functorially by left or right multiplication ($g\times \cdot$), call
this functor $\Gamma$.
...
-   there exists an inclusion from $\vert G \vert=\Gamma \bullet$ into
    $G = (\bullet)^{\bullet}$ which is natural and bidirectional

Functor (or action) $\Gamma$ is thus represented by $\bullet$.

### Pre - Yoneda lemma - $G$ equivariant maps determined by identity {#pre---yoneda-lemma---g-equivariant-maps-determined-by-identity beamer_opt="fragile"}
...
-   Consider two actions of $G$: $\Gamma$ (represented by $\bullet$, so
    $\Gamma g =
     g \times \cdot$) and $F$.

-   Any $G$ equivariant map $\alpha$ is completely determined by
    $\alpha(1)$ because $\alpha(g \times h) = g^*(\alpha(h))$ i.e.
    $\alpha(g) = g^*(\alpha(1))$

-   There is thus a bijection between $$Hom(\Gamma, F) \simeq
     Hom((\bullet)^\bullet,F) \simeq F\bullet$$

### Yoneda lemma [[focus]{.smallcaps}]{.tag tag-name="focus"} {#yoneda-lemma beamer_opt="fragile"}

Lemma\'s says how many natural transformations between a representable
presheaf and another presheaf $F$, i.e., the size of the set:

$$Hom((-)^c,F -) = \{ \alpha^0 \ldots \alpha^n \}$$

If $\alpha^i$ is a natural transformation then the following must
commute also for $x=c$ and $x^c=Id_c$:
...
$$\alpha^i_y (\phi \circ Id_c) \simeq \alpha^i_y(\phi) = (F \phi) (\alpha^i_c(Id_c))$$

Given $F$, fixed $\alpha^i_c(Id_c) \in Fc$ there is a single $\alpha^i$.
In practice: $$Hom((-)^c,F) = Fc$$

### Yoneda embedding [[focus]{.smallcaps}]{.tag tag-name="focus"} {#yoneda-embedding beamer_opt="fragile"}

Consider functors $(-)^a$ and $(-)^b$. The set of natural
transformations between them is, by Yoneda, isomorphic to $a^b$.

This can be seen as a contravariant functor in $a=*$ on its own (called
Yoneda embedding):

::: CENTER
```{=latex}
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRACoACAHW6zE4BhAHrAIaAL4gJpdJlz5CKMgEYqtRizYAjabJAZseAkRWk11es1aIQACgC0ASmG6Zco4tPl1VrbccXLl5+TgBlGBxhQWl1GCgAc3giUAAzACcIAFskMxAcCCRidxAM7KQAZmoCpAAmCQoJIA
  \begin{tikzcd}
  * \in C^{op} & (-)^* \in Set^C \arrow[d] \\
  b \arrow[u]  & (-)^b                    
  \end{tikzcd}
```
:::

which maps for any morphism in $C^{op}$ a natural transformation in
$Set^C$. This functor is fully faithful, i.e., when objects in $C$
become their hom-functors, connections between become natural
transformations. In Haskell, for any `b -> a` you have in
`(a -> x) -> (b -> x)` and viceversa.

### Yoneda embedding [[focus]{.smallcaps}]{.tag tag-name="focus"} {#yoneda-embedding-1}

-   Consider functors $(a)^-$ and $(b)^-$. The set of natural
    transformations between them is, by Yoneda, isomorphic to $a^b$.

-   The co-Yoneda embedding $C \rightarrow Set^{C}$ embeds $C$ in the
    category of pre-sheaves $Set^{C}$.

-   As an example, this can be used to show that in a preorder category
    $$a \leq b \textrm{~iff~} \forall x. x \leq a \implies x \leq b$$

### Functor representability as universal arrow from the singleton set [[focus]{.smallcaps}]{.tag tag-name="focus"} {#functor-representability-as-universal-arrow-from-the-singleton-set beamer_opt="fragile"}

Any **covariant** functor $(K-)^*:=K$ is **represented** by $r$ if
$(r, u: * \rightarrow Kr)$ is universal for $(K,*)$.
...
Yoneda: the components of the natural transformation $r^- \cong K-$ are
one to one with the elements of $Kr$.

### Universal arrow and adjunctions [[focus]{.smallcaps}]{.tag tag-name="focus"} {#universal-arrow-and-adjunctions beamer_opt="fragile"}

-   An adjunction is a universal arrow $(Lc, \eta_c)$ for $(R,c)$.

-   So, $(L*, \eta_*)$ is universal for $(R,*)$ so $R$ is representable
    by $L*$.

::: CENTER
```{=latex}
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZARgBoAGAXVJADcBDAGwFcYkQBjEAX1PU1z5CKcqWLU6TVuwBKAGS69+2PASIAmMRIYs2iEDKg8+IDCqFFR67VL0gFx5YLUpN1mjun6j3CTCgA5vBEoABmAE4QALZIAMw0OBBIACxKIBHRSGQgiUjqaRkxiKI5SYjZnnYAOlUwOPSO6ZFFJbmI+ZTcQA
\begin{tikzcd}
               & c \arrow[ld, "\eta"] \arrow[rd] &    \\
RLc \arrow[rr] &                                 & Rd \\
Lc \arrow[rr]  &                                 & d 
\end{tikzcd}
```
...
### Product as adjunction [[focus]{.smallcaps}]{.tag tag-name="focus"} {#product-as-adjunction beamer_opt="fragile"}

::: CENTER
```{=latex}
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
                                             & c \arrow[rd] \arrow[ld] &                        \\
c \times c \arrow[rr]                        &                         & a \times b             \\
{\Delta c = \langle c, c \rangle} \arrow[rr] &                         & {\langle a, b \rangle}
\end{tikzcd}
```
:::

$\Delta$ is left adjoint to $- \times -$.

### Cayley\'s through yoneda

-   Take $C = \{ * \}$ the category with a single object.

-   Consider that $F = (-)^*$ so we have that
    $$(*)^* \cong Nat((-)^*, (-)^*)$$.

-   But, $(*)^*$ is actually a group $G$ and it can be shown that
    $Nat(...)$ is actually a subgroup of permutations in $S_n$.

## Limits theory [[focus]{.smallcaps}]{.tag tag-name="focus"} {#limits-theory}

### Diagram functors {#diagram-functors beamer_opt="fragile"}

-   In any category $C$, I can pick objects by expressing a **pattern**,
    i.e., through another (small) **index** or **shape** category $J$
    and any functor $D: J \rightarrow C$ (diagram functor).

### Cone {#cone beamer_opt="fragile"}
...
\end{tikzcd}
```
-   A cone over a diagram $D$ is built over natural transformations
    $\lambda$ between the constant functor $\Delta_c$ and $D$ (both from
    the index category $S$).

-   Consider each family $\lambda_{c,-}$ of morphisms with apex $c$, we
    define $Cone(c,D)$ as the set $\{ \lambda_{c,-} \}$ (as there can be
    more than one family).

### Limit cone {#limit-cone beamer_opt="fragile"}
...
-   $Cone(-,D): C^{op} \rightarrow Set$ is a functor. If there exists an
    object $d
     \in C$ for which $Cone(-,D) \cong d^{(-)}$, we say that $d$ is the
    limit of $D$ and the cone that departs from it is the *limiting
    cone*.

-   In practice, if $d$ is a limit then for each morphism that arrives
    to it, there is a **single corresponding cone** from the sources of
    the morphism.

### Limit cone (in Sets) {#limit-cone-in-sets beamer_opt="fragile"}
...
-   Recall that if $d$ is a limit then for each morphism
    $m \in d^{(-)}$, there is a **single corresponding cone**
    $c \in Cone(-,D)$ (i.e., from the sources of the morphism).

-   In Set, if 1 is the singleton set we must have, e.g,
    $d^1 \cong Cone(1,D)$ which means that $d$ is $Cone(1,D)$. So
    $$Cone(-,D) \cong Cone(1,D)^{(-)}$$

### Products as limit {#products-as-limit beamer_opt="fragile"}
...
\end{tikzcd}
```
-   Consider a discrete category of only two object as the index
    category $S$.

-   Any functor $D: S \rightarrow C$ **indexes a generic pair** of
    objects $(i,j)$ (there are no choices in terms of morphisms to map)
    so we can speak of *the* functor $D$. MacLane goes as far as saying
    that $D$ **is a pair of objects** $(i,j)$.

-   A product object $i \times j$ is the limit of a diagram $D$ built
    over the above index category.

### Terminal object as limit {#terminal-object-as-limit beamer_opt="fragile"}
...
\end{tikzcd}
```
A terminal object is the limit of apices of cones built over an empty
index category.

### Equalizer as limit {#equalizer-as-limit beamer_opt="fragile"}
...
\end{tikzcd}
```
The equalizer of two morphisms $f,g$ is the limit of the above diagram.
In Set, it is a subset of $X$ such that $f(x) = g(x)$.

### Pullback (fibered product) {#pullback-fibered-product beamer_opt="fragile"}
...
\end{tikzcd}
```
-   The **pullback** **of two morphisms** $f,g$ is a limit object
    $lim~D$ and two morphisms $p,q$ such that $f \circ p = g \circ q$.
    In Set, consider that $lim~D$ is the set $Cone(1,D)$. The pullback
    would be $$A \times_C B = \{ (a,b) = f(a) = g(b) \}$$

-   If one uses fibers in $A$ and $B$ to make $f,g$ injective then
    $lim~D$ would be the product of fibers.

### Pushout (fibered sum) {#pushout-fibered-sum beamer_opt="fragile"}
...
\end{tikzcd}
```
-   The **pushout of two morphisms** $f,g$ is a co-limit object
    $colim~D$ and two morphisms $p,q$ such that the above commutes.
-   For sets, $colim~D$ is the disjoint union of $A$ and $C$ where
    elements of $A$ and $B$ that are the image of the same $c \in C$ map
    to the same element in $colim~D$ while the others map to different
    elements. It is a sort of fusion of $A$ and $B$ where some of the
    elements are collapsed into a single one.
-   When $f,g$ are inclusion maps (so $c \in C$ belongs to both $A$ and
    $B$), this is the canonical set union $A \cup B$.

### Fiber {#fiber beamer_opt="fragile"}

-   A **fiber** of a function $f: A \rightarrow B$ and a value $b \in B$
    is the set subset of $A$ such that:
    $$\phi_f(b) = \{ a | f(a) = b \}$$

-   It corresponds to the pullback of:
...
A lax closed functor (or **applicative**) $T$ is a functor for which
...
A lax monoidal functor $T$ is a functor for which there exist the
following natural transformations:

$$(\mu: Ta \times Tb \mto T(a \times b), u: T())$$

In a closed monoidal category one can derive a lax closed functor
(applicative) from this and viceversa.
...
-   An algebraic operation in domain $X$ is a morphism from tuple
    objects to $X$ e.g.: $$ X \times A
     \rightarrow X$$ or $$ A \rightarrow X $$

-   A co-algebraic operation in domain $X$ is a morphism from $X$ into a
    tuple object, possibly containing $X$: $$ X \rightarrow A \times
     X $$

### How to build morphisms of polynomial functors with respect to original morphism

-   If we can build functors for products and coproducts, we can build
    functors also for their combination;

-   For example: $T(X) = X + (C \times X)$ maps X to an object
    $X + (C \times X)$ but also morphisms $f: X \rightarrow X'$:
...
specified by $T$. For example, $$T(X) = 1 + X + (X \times X)$$ defines
$$e:
    1 \rightarrow U, i: X \rightarrow X, m: (X \times X) \rightarrow X$$
which might encode the signature of group operations.
...
Given two T-functor algebras $(U, a: T(-) \rightarrow -)$ and
$(V, b: T(-) \rightarrow -)$, a homomorphism of algebras from $(U,a)$ to
$(V,b)$ is a function $f: U \rightarrow V$ which commutes with the
operations $$ f \circ a = b \circ T(f)$$

### Initial algebras

-   Fixed a functor $T$, functor algebras $(U, a: T(U) \rightarrow U)$
    can be seen as categorical objects with algebra homomorphisms as
    morphism.

-   In this category, an **initial algebra** is an algebra for which
    there exist one and only one morphism from it to all the other
    algebras.
...
-   Initial algebras for some functors are already known. If we start
    from a generic functor $t$, the type of an initial algebra $\tia$
    should be such that $\tia \cong \tfun{\tia}$ (Lambeck).
-   Call $\tia \simeq Fix~t$ we have $Fix~t = t~(Fix~t)$, which in
    Haskell can be created through a recursive data type
    `Fix t = Fix t (Fix t)`. `Fix` encodes algebraic operations
    $\tfun{\tia} \rightarrow \tia$.
...
specified by $T$. For example, $$T(X) = A \times X$$ defines
$$(value,next):
    (U \rightarrow A, U \rightarrow U)$$ for any process $U$.

### Homomorphisms of co-algebras

Given two T-functor co-algebras $(U, a: - \rightarrow T(-))$ and
$(V, b: - \rightarrow T(-))$, a homomorphism of algebras from $(U,a)$ to
$(V,b)$ is a function $f: U \rightarrow V$ which commutes with the
operations $$ T(f) \circ a = b \circ f$$

### Final co-algebra

-   A final coalgebra $d: W \rightarrow T(W)$ is a coalgebra such that
    for every coalgebra $c: U \rightarrow T(U)$ there exists a unique
    map of co-algebras $f: (U,c) \rightarrow (W,d)$
...
## Notable F-algebra morphisms [[focus]{.smallcaps}]{.tag tag-name="focus"} {#notable-f-algebra-morphisms}

### Overview of (co-)algebra morphisms

::: footnotesize
Algebraic:

  --------------------------------------------------- ------------ -----------------------------------
  $I_t \rightsquigarrow Z$                            Cata         
  $I_t \rightsquigarrow I_t \times Z \rightarrow Z$   Para, Zygo   
  $I_t \rightsquigarrow C_t^A \rightarrow A$          Histo        C~t~^A^ is the annotation comonad
  --------------------------------------------------- ------------ -----------------------------------

Coalgebraic:

  ---------------------------------------------- ------ -------------------------------
  $Z \rightsquigarrow F_t$                       Ana    
  $Z \rightarrow Z + F_t \rightsquigarrow F_t$   Apo    
  $A \rightarrow M_t^A \rightsquigarrow F_t$     Futu   M~t~^A^ is the template monad
  ---------------------------------------------- ------ -------------------------------

Both:

  --------------------------------------------------------------------------------------- ----------------------------- -----------------------------------------------
  $A (\rightsquigarrow F_t/I_t \rightsquigarrow) B$                                       Hylo (Cata $\circ$ Ana)       
  $A (\rightarrow M_t^A \rightsquigarrow F_t/I_t \rightsquigarrow C_t^B \rightarrow) B$   Chrono (Histo $\circ$ Futu)   
  $A (\rightsquigarrow F_{(- + B) \circ t}/I_{t} \rightsquigarrow) B$                     Elgot                         Co-algebra collapses into B in case of errors
  --------------------------------------------------------------------------------------- ----------------------------- -----------------------------------------------
:::

### Catamorphisms {#catamorphisms beamer_opt="fragile"}
...
-   Given an initial algebra $(I, Fix: t~I \rightarrow I)$ and a generic
    algebra $(E, k)$, a generic morphism $g: I \rightarrow E$ can be
    defined as $g = k \circ (t~g) \circ unfix$
-   We say that $g = cata(k)$; given the same expression in $I$, I can
    create mappings into several other types by simply specifying the
    non-recursive $k$.

### Scan (as a catamorphism) {#scan-as-a-catamorphism beamer_opt="fragile"}

Given $\phi: A \times B \mto B$, $scan(\phi,b)$ is a catamorphism of
type:
...
### Catamorphisms fusion law {#catamorphisms-fusion-law beamer_opt="fragile"}
...
-   Given $g$ and $f$, if there exists an $h_{g,f}$ such that $g$ is an
    algebra morphism, the composition of $g \circ \kappa(f)$ is equal to
    $\kappa(h_{g,f})$.

-   For example, for $I_t$ a list, $\kappa(f) = fold (+1) = length$ and
    $g = (2*)$ then $(2*) \circ
     length$ can be rewritten as the fold of $(+n)$, instead of $(+1)$.

-   For a list this happens if one can find a $h_{g,f}$ such that
    $$(g \circ
     f)(a,n)=h_{g,f}(a,f(n))$$

### Catamorphism compose law {#catamorphism-compose-law beamer_opt="fragile"}
...
-   If $I$ is an expression type, and $h$ and $f$ are optimizations
    expressed as F-algebras, you can do a single optimization pass by
    composing their algebras: Another example,
-   Or, $h$ could filter away data.

### Combining algebras $(A, \phi), (B, \psi)$ into $(A \times B, q$ (banana split) {#combining-algebras-a-phi-b-psi-into-a-times-b-q-banana-split beamer_opt="fragile"}

Given $\phi: \tfun{a} \rightarrow a$ and $\psi: \tfun{b} \rightarrow b$
produce $\beta: \tfun{(a \times b)} \rightarrow (a \times b)$ as
...
### Hylomorphisms {#hylomorphisms beamer_opt="fragile"}

-   Given a initial and final algebra of the same functor $t$
    $$hylo(\psi,\phi) = cata(\phi) \circ ana(\psi)$$

-   This is the basis for deforestation, it eliminates intermediate data
    structures.
...
-   allows to exploit parallelism? substituting recursive control with a
    data structure

### Hylomorphism example: mergesort {#hylomorphism-example-mergesort beamer_opt="fragile"}
...
-   Consider the functor $t_A[X] = A + X \times X$, its initial algebra
    $\tia$ being a binary tree of $A$\'s.
-   Consider $L_A$ as a list of A\'s. One could implement merge sort
    $\mu: L_A
     \rightarrow L_A$ as $hylo(\psi,\phi)$, where $\phi$ constructs the
    tree by splitting lists in two while $\psi$ merges two lists into
    one (by reordering them).

### Hylomorphism, optimized {#hylomorphism-optimized beamer_opt="fragile"}

This version of hylomorphism $y$ is optimized because it calls O(n)
times fmap instead of O(2n).

```{=latex}
% https://q.uiver.app/?q=WzAsNCxbMCwxLCJBIl0sWzAsMCwidEEiXSxbMSwwLCJ0QiJdLFsxLDEsIkIiXSxbMCwzLCJ5Il0sWzAsMSwiXFxhbHBoYSIsMl0sWzEsMiwiVHkiLDJdLFsyLDMsIlxcYmV0YSIsMl1d
\[\begin{tikzcd}
    {tA} & {tB} \\
    {A} & {B}
    \arrow["{y(\alpha, \beta)}", from=2-1, to=2-2]
    \arrow["{\alpha}"', from=2-1, to=1-1]
    \arrow["{Ty}"', from=1-1, to=1-2]
    \arrow["{\beta}"', from=1-2, to=2-2]
\end{tikzcd}\]
```
### Elgot algebras {#elgot-algebras beamer_opt="fragile"}

Useful when, in the course of an hylomorphism we want to short circuit
into

1.  In this case $\alpha$ is an elgot co-algebra, and $\beta$ is an
    elgot algebra.

```{=latex}
% https://q.uiver.app/?q=WzAsNCxbMCwwLCJCK3RBIl0sWzAsMiwiQSJdLFsyLDAsInRCIl0sWzIsMiwiQiJdLFswLDIsIlRcXGVwc2lsb24iLDJdLFsyLDMsIlxcYmV0YSIsMl0sWzEsMywiXFxlcHNpbG9uIChcXGFscGhhLCBcXGJldGEpIl0sWzAsMywiaWQgXFxuYWJsYSAoXFxiZXRhIFxcY2lyYyBUXFxlcHNpbG9uKSIsMV0sWzEsMCwiXFxhbHBoYSIsMl1d
\[\begin{tikzcd}
    {B+tA} && {tB} \\
    \\
    {A} && {B}
    \arrow["{T\epsilon}"', from=1-1, to=1-3]
    \arrow["{\beta}"', from=1-3, to=3-3]
    \arrow["{\epsilon (\alpha, \beta)}", from=3-1, to=3-3]
    \arrow["{id \nabla (\beta \circ T\epsilon)}" description, from=1-1, to=3-3]
    \arrow["{\alpha}"', from=3-1, to=1-1]
\end{tikzcd}\]
```
### Paramorphisms {#paramorphisms beamer_opt="fragile"}
...
Example, for factorial $f(i,z) = i~?~z*i:1$. Exercise, try to devise a
para for sliding window.

### Zygomorphism {#zygomorphism beamer_opt="fragile"}
...
### Histomorphism (on annotations) {#histomorphism-on-annotations beamer_opt="fragile"}
...
Applies annotations to the cofree comonad, folding them from leafs
through $\phi: \tfun{\tcfcm} \mto A$ and returns the one associated with
the top of the tree. E.g., for Fibonacci:
...
\end{equation}
```
## Expression data types as functors [[focus]{.smallcaps}]{.tag tag-name="focus"} {#expression-data-types-as-functors}

### Expression data types as initial alg. of functors {#expression-data-types-as-initial-alg.-of-functors beamer_opt="fragile"}

Two ways to build them,

1.  recursive data type which needs an explicitly recursive function to
    interpret it.
...
-   Using a non-recursive data type
    ...
    \end{tikzcd}
    ```
    Build expressions in `Expr` with original type constructors,
    evaluate with `eval
     = cata rules`.

### Expression data types as functors - templating (free monad) {#expression-data-types-as-functors---templating-free-monad beamer_opt="fragile"}

-   If $I_t$ is an initial algebra representing an AST of expressions
    built with $t$ then the fixed point $M_t^A$ of $(A + t[-])$ is a
    ****tree similar to $I_t$ with holes of type \$A\$**** while $T$
    merges them into the tree itself. Such a monad can be easily used as
    a co-algebra for building streams (see futumorphisms).

-   Consider $A$ = String referencing the name of a variable. Then, in
    the following picture, $\phi: A \rightarrow I_t$ can be used to
    build a complete tree $I_t$ by substituting instances of $A$ with
    subtrees $I_t$:
...
### Expression data types as functors - stripping annotations (cofree comonad) {#expression-data-types-as-functors---stripping-annotations-cofree-comonad beamer_opt="fragile"}
...
-   The cofree comonad is the fixed point $\tcfcm$. It is a tree similar
    to $\tia$ where all nodes have been annotated with values of type
    $A$.

-   $\beta: \tfun{\tcfcm} \times A \mto \tcfcm$ allows to build a new
    tree from child trees and an annotation. $\beta = Fix \circ AnnF$

-   $\tcfcm$ is an initial algebra as well. As the free monad, it can
    give back the original $\tia$ with $\kappa(\phi)$ (stripping
    annotations).

### Expression data types as functors - adding annotations bottom-up {#expression-data-types-as-functors---adding-annotations-bottom-up beamer_opt="fragile"}
...
-   $\alpha$ allows to get an annotation from the root of the tree
    $\tcfcm$, while $f: \tfun{A} \mto A$ is a $t$ algebra that allows to
    compute a new $A$ from the set of underlying tree\'s $A$;
-   $\kappa (\beta \circ ...)$ can be used to annotate a tree; for
    example $f = (+1) \circ
     sum_T$ can be used to annotate a root with the sum of $T$ childs
    (bottom-up annotation). Note the similarities with histomorphisms

### Expression data types as functors - evaluating into a monad {#expression-data-types-as-functors---evaluating-into-a-monad beamer_opt="fragile"}

Useful to attach monadic actions to expressions; in general you would
need an algebra `f (m a) -> m a` but that would be difficult to specify.
You can do instead with `alpha: f a -> m a`, using `cataM` instead of
`cata` and using kleisli arrow composition.

::: CENTER
```{=latex}
% https://tikzcd.yichuanshen.de/#-sQ9ofTqBiwwQLgIF6gQalaYOm+Bkgbx+7DoWAYbGBrGedAARjAGAAFYSqMQgBgweKKCh8IA
\begin{tikzcd}
tI_t \arrow[r, "t_M (\kappa_M(\alpha))"]              & M_{tA} \arrow[d, "M\alpha"] \\
                                                      & M^2_A \arrow[d, "\mu"]      \\
I_t \arrow[r, "\kappa_M(\alpha)"] \arrow[uu, no head] & M_A                        
\end{tikzcd}
```
:::
...
### Parser combinators {#parser-combinators beamer_opt="fragile"}

-   A *parser* of A ($P[A]$), is an object $A^S$ which takes a string
    (type $S$) and returns a value $A$.

-   A parser combinator, is a *higher order function* that takes two or
    more parsers and returns a new parser (e.g.,
    $alt: P[A]\times P[B] \rightarrow P[A+B]$ and
...
-   $P[A]$ is a functor in $A$; You can get a parser $PB$ for object $B$
    if you have $P[A]$ and $A \rightarrow B$.

### Expression data types as functors - parsers (annotation) {#expression-data-types-as-functors---parsers-annotation beamer_opt="fragile"}

Each constructor in $t$, e.g., $n_2: X \times X \mto t[X]$ can be made
into a parser of annotated trees by introducing a function $inj$ that,
from a tree parser creates an annotated tree parser.
...
## Functor co-algebras [[focus]{.smallcaps}]{.tag tag-name="focus"} {#functor-co-algebras}

### Anamorphisms

-   Given a final co-algebra A, and a generic co-algebra $(B,f)$ the
    unfold of $f$ allows to create a mapping $B \rightarrow A$
    non-recursively; this is called $ana(f)$.

-   Anamorphisms are called co-recursion.

### Apomorphisms {#apomorphisms beamer_opt="fragile"}
...
Given a co-algebra $\phi: C \mto t (C + \fia)$ where $\fia$ is final, it
allows to build a morphism $C \rightarrow \fia$ that short-circuits the
traversal by giving the result immediately.

### Apomorphisms example: streams {#apomorphisms-example-streams beamer_opt="fragile"}
...
$\phi: A \mto A \times A + A \times S_a$ so it can return $A \times A$
(becoming in the recursion, a generator) or $A \times S_A$ and from that
point the $cofix: S_A \mto A \times S_A$ starts. It corresponds to a
transformation identity so it does nothing.

### Futumorphisms {#futumorphisms beamer_opt="fragile"}

A futumorphism $u(\phi)$, given an $\phi: A \rightarrow tM_t^A$, allows
to create a co-algebra $M_t^A \rightarrow tM_t^A$ which can be used to
construct a co-inductive value (e.g., a stream). If $A$ is $F_t$, then
$\phi$ could access future values to produce the current one.
...
### Futumorphisms (stream example) {#futumorphisms-stream-example beamer_opt="fragile"}

Assume $S$ is a stream of $A$\'s and ($t$ is the co-algebra). $\phi(S)$
can take future values in $S$ (by destructuring) and returning them as
$A$ and decide what is the next $M_t^S$ to be used (the second element
of the pair).
...
### Streams as co-inductive data types (generation) {#streams-as-co-inductive-data-types-generation beamer_opt="fragile"}

A stream $S_A$ is a final co-algebra of $s \mapsto a \times s$. One can
build a stream from a seed value $a \in A$ by using
$ana(\langle Id, \nu \rangle)$ where $\nu$ specifies how to generate the
next value.
...
### Streams as co-inductive data types (transformation) {#streams-as-co-inductive-data-types-transformation beamer_opt="fragile"}
...
$\nu$ can be lifted into a stream transformer:
$\phi(s) = \nu_A(s) \cdot \phi(\nu_S(s))$, for example
$\nu(s)= \langle (head(s))+1, tail(s) \rangle$ adds one to the stream.

## Proofs with functor algebras

### An initial algebra defines homomorphisms by induction (integers)

-   Let\'s indicate with $(N, [0,S]: 1 + N \rightarrow N$) the
    **initial** algebra of natural numbers.
...
### An initial algebra defines homomorphisms by induction (lists) focus:

-   Let\'s indicate with
    $(A^*, [e,c]: 1 + A \times A^* \rightarrow A^*$) the **initial**
    algebra of lists of $A$.

-   An algebra $$(Q^*, [q_e,q_c]:
     1+A \times Q^* \rightarrow Q^*)$$ defines, by induction, one and
    only one algebra morphism $f: A^* \rightarrow Q^*$.

    -   $f \circ e = q_e$
    -   $f \circ c = q_c \circ (id \times f)$

-   For example, an algebra $$(\mathbb{N}, [0, S \circ \pi]: 1+A \times
     \mathbb{N} \rightarrow \mathbb{N})$$ defines implicitly the length
    of a list

### Proofs by induction with initial algebras (integers) {#proofs-by-induction-with-initial-algebras-integers beamer_opt="fragile"}
...
-   ($0 \in P$ and $n \in P \Rightarrow (n+1) \in P$) means there exists
    an algebra for $P$.

Then, $i$ is an algebra morphism because the square commutes. Since
there is only algebra morphism from $N$ to $N$ (id) this means that $i$
must be an epi-morphism.

### Proofs by induction with initial algebras (lists) {#proofs-by-induction-with-initial-algebras-lists beamer_opt="fragile"}
...
Then $i$ must belong to the unique identity morphism on $A^*$ so it is
an epimorphism.

### Bisimulation (for lists) [[focus]{.smallcaps}]{.tag tag-name="focus"} {#bisimulation-for-lists beamer_opt="fragile"}
...
-   $R$ is called **a bi-simulation** on $S$, if there exists
    $(R,\gamma)$ for which $\pi_1, \pi_2$ are algebra homomorphisms from
    $(R,\gamma)$ to $(S, \xi)$.

-   If $S$ is a final co-algebra, e.g., a list $\xi = [h,t]$ then
    $\pi_1$ and $\pi_2$ map to the same list $a=b$. If you prove the
    existence of $\gamma$ then the bisimulation $R$ is an equivalence.
    For lists $\gamma$ must abide by:
...
### Bisimulation (labeled transition system) [[focus]{.smallcaps}]{.tag tag-name="focus"} {#bisimulation-labeled-transition-system beamer_opt="fragile"}

A labeled transition system $(S,A,\rightarrow)$ is a co-algebra
$$\alpha: S
\rightarrow P(S)^A$$ i.e., to any $s \in S$ it associates any input
symbol $a \in A$ to multiple $t \in S$ such that $s \rightarrow^a t$
belongs to the system itself. If $P(S)^A$ is finite then a final
co-algebra exists; it is the algebra of processes $(P, \pi)$

## Free monads [[focus]{.smallcaps}]{.tag tag-name="focus"} {#free-monads}

### Free monads (construction) {#free-monads-construction beamer_opt="fragile"}
...
-   The **free monad** $\tfm$ is the fixed point of a functor where
    $Fix = \pi + \phi$ (pure and free).

### Free monads (bound function) {#free-monads-bound-function beamer_opt="fragile"}
...
-   binding a function $f$ (i.e., $f^o$) depends on whether its argument
    comes from a pure $\pi$ or a free $\phi$:
    $$f^o = \begin{cases} f \circ \pi^{-1}
     \\ \phi\circ Tf^o \phi^{-1}\\ \end{cases}$$

### Free monads (underlying commands from functor)

-   Assume $C$ is the type of some sequence of commands; the functor
    represents the composition of one action with that sequence
    $$T[C] = (S \times
     C)_{print} + (S \mto C)_{getline} + 1_{end}$$

    ::: CENTER
    ```{=latex}
    \includegraphics[width=4cm]{./key/page-4-crop.pdf}
    ```
    :::

-   It is a way to build backwards a sequence of commands, e.g.,
    $$getline~(x
     \mapsto (print~("hi" x) (end))) ~~~ : T[T[T[C]]]$$

### Free monads (building monad commands from functor ones) {#free-monads-building-monad-commands-from-functor-ones beamer_opt="fragile"}

Remember that $\tfun{A}$ is a sequence of commands. To build the AST
tree through a Free monad, e.g., `type Console = Free ConsoleF`, we need
to lift single commands as they were sequences of one command alone.
...
### Free monads (composition) {#free-monads-composition beamer_opt="fragile"}
...
This is a way to compose functors which manage different effects into a
single monad.

### Free monads (interpretation) {#free-monads-interpretation beamer_opt="fragile"}

An interpreter $\iota$ between a free monad $M_T^A$ and a monad $qA$
passes through the free monad built on $q$. It can be built through
$\nu$ (for which two appropriate morphism $\lambda$ and $\mu$ must be
provided). $\lambda$ must abide the laws on the right for any $\nu$.

::: CENTER
```{=latex}
% https://tikzcd.yichuanshen.de/#-QGrRA6DZbRAANnBQUO+Bw9BhVxu926wiRKPRMKwYCCUHocFRE06WKQuNBiDIIFh1IRdOer3eA3Yhwuy0ubIAFIcwMwAJTcpbYwX8om7En7ZbMSlwmkRdjijGWHmIep47bE6X7WXy+hK-Yq9XnKnw2nsZFoy3zTVIACsIPxuqlRmVRq9JtF5pebxonPo3xKAHcIKmoAhzszWezOd9McHEAB2cNgvWOtCok7JoXe001IVJjVAitVxAADgdJRl9Dlipjnqb8d9T3bpc7fbtiAAnP2Du7Y+ORZO2xKZ9il-OdlGV3WTiZjRuzVPt5RuEA
\begin{tikzcd}
                                                             &                        & A \arrow[rrd, "\pi_q", bend left] \arrow[d, "\pi_t"']     &                             &                                \\
M_t^A \arrow[rd, "\iota"', dashed] \arrow[r, "\lambda(\nu)"] & M_q^A \arrow[d, "\mu"] & M_t^A \arrow[rr, "\lambda(\nu)"']                         &                             & M_q^A                          \\
tA \arrow[r, "\nu", two heads, dashed]                       & qA                     & {t[M_t^A]} \arrow[u, "\phi_t"] \arrow[r, "t\lambda(\nu)"] & {T[M_q^A]} \arrow[r, "\nu"] & {q[M_q^A]} \arrow[u, "\phi_q"]
\end{tikzcd}
```
:::
...
-   A function `(a -> r) -> r` is called a suspended computation (SC).
    It needs a way to transform its intermediate value `a` into its
    final result `r`.
-   Such a way is provided by means of a continuation `a -> r`.
-   SCs form an endofunctor $Hom(Hom(-,R),R)$; this endofunctor can be
    made into a monad where a new `SC b` can be built from an `SC a` by
    specifying `a ->
     SC b` (bind).

# Logic,computation theory, dependent types

## Dependent types [[focus]{.smallcaps}]{.tag tag-name="focus"} {#dependent-types}
...
-   `Bool` is a kind which corresponds to a set of two sets (types)
    `'True` and `'False`. We can\'t however pattern match on those. To
    enable matching they must be mapped to a singleton type with is own
    value constructor.

### Dependent product

A dependent product is a function from a value $a : A$ into a value
$b: B(a)$: $$ \lambda x.\Phi : \Pi_{x:A} B_x $$

-   If $A = \{ x_1 \}$ were a singleton type the function would have a
    constant type: $B_{x_1}^{\{x_1\}}$.

-   If $A$ were more than one element we would end up with:
    $$ \Pi_{x:A} B_x =
     B_{x_1}^{\{x_1\}} \times B_{x_2}^{\{x_2\}} \times \ldots $$

-   If $B$ were constant, we would obtain a simple exponential
    $$B^{\sum x_i} \sim
     B^A$$

### Fixed point {#fixed-point beamer_opt="fragile"}

A fixed point $\bar{x}$ of a function $f$ is such that the following
commutes:
...
### Fixed point combinator {#fixed-point-combinator beamer_opt="fragile"}

When $f$ is interpreted as a function between functions $(X^N)^{X^N}$,
the fixed point is another function $\phi \in X^N$ such that the below
commutes:
...
For example, if $f \gamma n = (n>0)~?~(n*\gamma(n-1))~1$, then
$$fix~f= f~(fix~
f)~~~\simeq~~~\phi~n = (n>0)~?~(n*\phi(n-1))~1$$

For languages that do not support recursion but only higher order
functions, there is an explicit lambda representation for $fix$ found by
H.Curry.

### Kinds

-   The kinds in Haskell are: `type` (asterisk) and `constraint`.

-   Higher kinded datatypes are mappings: `type -> type`, e.g. Maybe.

-   We can create new kinds with `-XDataKinds`. Every type declaration
    becomes a kind declaration (with ticks):

    ``` example
    data Bool = True | False
    ```

    produces kind `Bool` and types `'True` and `'False`. Strings are
    promoted to kind `Symbol`.
...
The promotion -`XDataKinds` creates a kind `[type]`; one can create a
type which is a list of types through the promoted constructors, e.g.,
...
With `-XTypeApplicatons`

``` example
fmap :: Functor f => a -> b -> f a -> f b 
fmap @Maybe :: a -> b -> Maybe a -> Maybe b
fmap @_ @Int @Bool :: Int -> Bool -> f Int -> f Bool
```

Can be used with `-XAllowAmbiguousTypes` to define functions whose type
parameters are only on the left of the fat arrow `=>`:
...
With `-XGADTs` you get type equalities; the GADTS
...
## Logic [[focus]{.smallcaps}]{.tag tag-name="focus"} {#logic}

### Classical logic

-   it is *truth functional*, i.e., every complex proposition truth\'s
    is function of the truth of inner proposition, regardless of the
    fact that we can find witnesses for them.

-   In classic logic the following is a valid argument $$\neg(\forall x,
     \neg P(x)) \vdash \exists x.P(x)$$ This depends on the principle of
    the third excluded which is not a valid rule in, for example,
    constructive logic.
...
-   An existential can only be proven if I provide a witness:
    $$P(a) \vdash
     \exists x.P(x)$$

-   It is classic logic without some axioms, for example $A \vee \neg A$
    is not a valid axiom in .

### Structural proof theory

-   It studies the validity of an argument $X \vdash Y$ from its
    structure
...
### Proof theory as a monoidal theory

-   Given an argument $X \vdash Y$, the proof of its validity is seen as
    the existence of one morphism between objects $X$ and $Y$.
    $X \vdash Y$ is then understood as the homset of morphisms from $X$
    to $Y$.

-   $X \implies Y$ is an object corresponding to the set of equivalence
    classes of proofs from $X$ to $Y$. $X \otimes Y$ is the proposition
    $X \wedge Y$.

-   An **inference rule** specifies the validity of the inferred
    argument from that of the premises. It is a **morphism between
    homsets** which is always present.

### Monoidal proof theory inference rules

  ------------- -------------- ---------------------------------------------------------------
  CC name       Logic name     Inference rule
                               
  composition   cut            `\infer{X \vdash Z}{X \vdash Y & Y \vdash Z}`{=latex}
                               
  identity      identity       `\infer{X \vdash X}{}`{=latex}
                               
  Currying                     `\infer{Y \vdash X \implies Z}{X \otimes Y \vdash Z}`{=latex}
                               
                Modus ponens   `\infer{X \otimes (X \implies Y) \vdash Y}{}`{=latex}
  ------------- -------------- ---------------------------------------------------------------

### Structural proof theory examples

-   The most widespread are: Hilbert\'s calculi, Gentzen natural
    deduction, Gentzen sequent calculus
...
-   Many axioms schemes, where each scheme is a template, e.g.,
    $\vdash \phi \mto \phi$ can mean $\vdash p \mto p$ or
    $\vdash (p \mto p) \mto (p \mto p)$
-   one inference rule $$\infer{(X, X \mto Y) \vdash Y}{}$$
-   Every line is an unconditional tautology.
-   classic logic has an axiom scheme
...
-   Introduces the concept of sequent:
    $$ X_1, \ldots X_n \vdash Y_1 \ldots Y_m $$ which is a proposition
    that is read as: all of $X_i$ can be used to derive the truth of at
    least one of $Y_j$
-   Every line is a conditional tautology with zero or more conditions
-   Every (conditional) line has zero or more asserted propositions on
    the right.

### Natural deduction

-   It is a sequent calculus where every (conditional) line has exactly
    one asserted proposition on the right.

### $\wedge$ introduction

-   It is a valid inference rule for natural deduction
    $$\{ A, B \} \vdash A \wedge B$$ or $$\infer{A \wedge B}{A & B}$$

-   Example of application
    $\{ A, B, C \} \vdash A \wedge B \wedge A \wedge C$:
...
\end{equation}
```
Negation elimination is valid only in classical logic through double
negation
...
\end{equation}
```
This is exactly the rule of the excluded middle that was argued by
Brower in 1907.

### Boolean algebra

It is a lattice where every element $a$ has *complement* $\bar{a}$,
i.e., such that $$a \vee \bar{a} = 1$$ and $$a \wedge \bar{a} = 0$$ and
where meets and joins are distributive.

### Heyting algebra

-   A Heyting algebra is a bounded lattice with exponential objects
    $X \implies Y$
-   It is a Boolean algebra minus the complement for $\vee$.
-   The set of propositions $P$ with implication as morphisms is a
    Heyting Algebra because:
    -   $X \wedge Y$ behaves as the product and $\top$ is the terminal
        object.

    -   the proposition $X \implies Z$ is another object and is such
        that is such that
...
-   If $x$ is a variable and $t$ is a term, then $(\lambda x .t)$ is a
    term (*lambda*-abstraction)

From a simple set of rules, Church and Kleene were able to build up
Boolean logic, the natural numbers, the usual operations of arithmetic,
and so on. All computable functions $f: N \mto N$ can be defined in the
lambda calculus

### Lambda calculus (beta and eta reduction)

If we apply $\lambda x.t$ to $s$ we get back $t$ but with $s$
substituted for each free occurrence of $x$:
$$(\lambda x.t)(s) = t[s/x]$$ This is a rewrite rule (beta reduction);
by repeatedly applying it we perform **computation**. Eta reduction says
instead that one can always rewrite $$(\lambda x. t(x)) = t$$ Alpha
conversion allows to rename the bound variable, e.g., $x$ in
$\lambda x.t$.
...
then $$ (\lambda a.(\lambda b.(\lambda x.a(b(x)))))$$ can define
multiplication by repeatedly applying rewrite rules.

### Typed lambda calculus

When types are added to lambda calculus, terms (programs) can be seen as
morphisms in a bicartesian closed category where data types are objects.

-   A term from $X$ to $Y$ is an element of $Y^X$ which is a function
    type. Since two terms might be actually the same morphism by rewrite
    rules, this must be intended as a set of equivalence classes under
    beta reduction.

-   A product type is a pair of two types and there is also a terminal
    object (unit type) with only one element.

-   See the bicartesian closed category Set

# Cryptography, quantum, probability, statistics

## Quantum computing [[focus]{.smallcaps}]{.tag tag-name="focus"} {#quantum-computing}

### Pauli matrices

::: CENTER
```{=latex}
\includegraphics[width=1\linewidth]{./images/q_pauli.pdf}
```
:::

### Hilbert space $C^n$ - inner product

$C^n$ is an Hilbert space where the inner product is defined by:

::: CENTER
```{=latex}
\includegraphics[width=1\linewidth]{./images/q_hilb.pdf}
```
:::

The inner product of two vectors is equal to the vector inner product
between two matrix representations of those vectors.

### Hilbert space $C^n$ - outer product

Remember that $\braket{v}{v'}$ is a complex number. The outer product
operator $\ketbra{w}{v}$ is a matrix defined as:

$$(\ketbra{w}{v})\ket{v'} = \braket{v}{v'}\ket{w}$$

If $\ket{i}$ is an orthonormal basis, we have that (*completeness
relation*): $$\sum_i \ketbra{i}{i} = I$$
...
$$A \ket{v} = v \ket{v}$$ where $v$ is the eigenvalue of $A$
corresponding to $\ket{v}$ (note that we will use the same name but the
type is different. Eigenvalues of $A$ can be found by solving
$$c(v) = det | A - vI | = 0$$

### Diagonal representation of an operator

An operator $A$ is diagonalizabile if it can afford a representation in
terms of outer products of eigenvectors:

$$A = \sum_i \lambda_i \ketbra{i}{i}$$

for example Pauli\'s $Z = \ketbra{0}{0} - \ketbra{1}{1}$ is thus
diagonalizable.

### Qubit

A qubit is something that has a state which, when measured can be either
0 with probability $\alpha$ or 1 with probability $\beta$. We describe
this state in a vector space of basis states:
...
The state of a qubit can be alternatively described with two parameters
$\theta$ and $\phi$ which are the possible angles of the unit vector.

### Bell state

A Bell state (EPR) pair is defined as:
$$\ket{\beta_{00}} = \frac{\ket{00} + \ket{11}}{\sqrt{2}}$$ In a Bell
state, if one measures only 1 of the two qubits, the other is
determined.

### Quantum gate

-   A quantum gate operates on a set of qubits by a linear operation in
    the state vector space whose basis are the possible combinations of
    bits.

-   This matrix $U$ must be such that $UU^+=I$ because they must
    preserve probability. Since unitary matrices are invertible, this
    means that all quantum gates must be reversible.

-   There are infinitely many two by two unitary matrices, properties of
    the complete set can be under- stood from the properties of a much
    smaller set called universal set.

### Quantum not gate

A 1 to 1 quantum gate swaps the basis states of a qubit:
$$a\ket{0} + b\ket{1} \mto
a\ket{1} + b\ket{0}$$

### Quantum Hadamard gate

A 1 to 1 quantum gate that puts in superposition the basis states of a
qubit: $$a\ket{0} + b\ket{1} \mto a\frac{\ket{0} + \ket{1}}{\sqrt{2}} +
b\frac{\ket{0} + \ket{1}}{\sqrt{2}}$$

For example, applying $n$ Hadamard gates to $n$ qubits in the state
$\ket{0}$ provides a final state of:
$$H^{\otimes n}(\ket{0}) = \ket{0}^{\otimes n} = \frac{1}{\sqrt{2^n}}\sum_x \ket{x}$$

### Quantum - Controlled not gate (CNOT)

It is a 2 qubit quantum gate which rotates values in the state vector
space with the following law:

::: CENTER
```{=latex}
\includegraphics[width=0.35\linewidth]{./images/q_cnotgate.pdf}
```
:::

In practice, it inverts the components of $\ket{10}$ and $\ket{11}$. Any
multiple qubit gate can be composed by a CNOT and single qubit gates. If
the state is a pure computational basis state $\ket{a,b}$ it is
transformed into the basis state $\ket{a, a \oplus b}$.

### Quantum teleportation

If two points $A$ and $B$ share a bell state, they can exchange quantum
information through classic bit communication:

::: CENTER
```{=latex}
\includegraphics[width=0.85\linewidth]{./images/q_teleportation.pdf}
```
:::

::: CENTER
```{=latex}
\includegraphics[width=0.55\linewidth]{./images/q_teleportation_psi2.pdf}
```
:::

Which limits B\'s to 4 possible states after A performs measurements
$M_1$ and $M_2$.

### Toffoli gate

The Toffoli gate has three input bits and three output bits. Two of the
bits are control bits that are unaffected by the action of the Toffoli
gate:

::: CENTER
```{=latex}
\includegraphics[width=0.35\linewidth]{./images/q_toffoli.pdf}
```
:::

### Ancilla state

It is a standard state prepared to create new gates, e.g., 1 is used in
this Toffoli gate to create a NAND gate:

::: CENTER
```{=latex}
\includegraphics[width=0.55\linewidth]{./images/q_ancilla.pdf}
```
:::

### Classic quantum parallelism (1 bit)

Assume $x$ the data register and $y$ the data register are both qubits
and define a unitary transformation
$U_f:  \ket{x,y} \mto \ket{x,y \oplus f(x)}$. If the data register is a
superposition, the final state will be in a superposition as well

::: CENTER
```{=latex}
\includegraphics[width=0.55\linewidth]{./images/q_parallelism.pdf}
```
:::

I.e., $$\ket{\psi} = \frac{\ket{0,f(0)} + \ket{1,f(1)}}{\sqrt{2}}$$

### Quantum parallelism ($n$ bit)

Applying $n$ Hadamard gates to $n$ qubits in the state $\ket{0}$
provides a final state of:
$$H^{\otimes n}(\ket{0}) = \ket{0}^{\otimes n} = \frac{1}{\sqrt{2^n}}\sum_x \ket{x}$$

If we apply a $U_f$ (where $f$ is an $n$ to $1$ bit function) to the
state $\ket{0}^{\otimes n}\ket{0}$, the resulting state is
$$\frac{1}{\sqrt{2^n}}\sum_x \ket{x}\ket{f(x)}$$

### Quantum parallelism ($1$ bit) - Deutsch\'s algorithm

With two additional hadamard gates with respect to the classic
parallelism gate:

::: CENTER
```{=latex}
\includegraphics[width=0.55\linewidth]{./images/q_deutsch.pdf}
```
:::

The first qubit of $\ket{\psi_3}$ will be proportional to
$f(0) \oplus f(1)$ thus allowing us to determine some global property of
$f$ with just one evaluation of $f$.

### Quantum parallelism ($n$ bit) - Deutsch-Josza algorithm

::: CENTER
```{=latex}
\includegraphics[width=0.55\linewidth]{./images/q_deutsch_josza.pdf}
```
:::

The first $n$ qubits are
$\sum_{\gamma} \widehat{f}(\gamma)\ket{\gamma}$. The probability of
measuring a $\ket{0...0}$ is $\widehat{f}(0)^2$, so, in the case where
$f$ can be either balanced or constant, we can distinguish the two
because, in the first case, one would never measure such state while in
the second case one would always measure such state.

## Probability and statistics [[focus]{.smallcaps}]{.tag tag-name="focus"} {#probability-and-statistics}
...
Roughly, is a weighted average of the difference in terms of bits for
the encoding of a set of data following optimizied for $p$ and $q$
respectively.

### Mutual information

It is related to KL. It\'s the number of bits you save representing X
and Y if you understand the relationship between them instead of
